...
export default function Home() {
  ...
  useEffect(() => {
  getHolidays(currentDate.getFullYear()).then((data) => {
  ...
  return (
    <main
      className="min-h-screen flex items-center justify-center bg-[#e8e4dc] p-4 md:p-8"
      style={{ fontFamily: "'Georgia', 'Times New Roman', serif" }}
    >
      {/* Outer calendar card */}
      <div
        className="w-full max-w-5xl calendar-card rounded-2xl overflow-hidden shadow-2xl"
        style={{
          background: "#fff",
      ...
      >
        ...
        <div
          className="spiral-strip w-full flex items-center justify-center"
          style={{
            height: "28px",
        ...
        <div className="grid grid-cols-1 md:grid-cols-2">
          ...
          >
            ...
            {/* Divider */}
            <div
              style={{ height: "1px", background: "#e2ddd6", margin: "0 20px" }}
            />

            {/* Notes panel */}
            <div className="p-4 md:p-5">
              <NotesPanel range={range} notes={notes} setNotes={setNotes} />
            </div>
          </div>
        </div>
      </div>
